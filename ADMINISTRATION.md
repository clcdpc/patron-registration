...
Distinct nonblank codes already present in `RegistrationFormSettings` are included even when metadata has never been deployed, including `kiosk`. They appear as legacy/unregistered and can be explicitly adopted into authorized system or library metadata. Branch-owned rows are mapped to their library with the existing organization cache. Adoption creates metadata only and never copies inherited settings.
